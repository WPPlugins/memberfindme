...
function sf_widget_folder_do($instance,$set) {
	...
	if (!empty($dat)) foreach($dat as $x) {
		if ($instance['act']=='1')
			...
			$out[]='<li><a href="'.esc_attr($x['url']).'">'
				.($x['lgo']&&!empty($instance['lgo'])?('<div class="member-image"><img src="//cdn.membershipworks.com/u/'.$x['_id'].'_lgl.jpg?'.$x['lgo'].'" alt="'.esc_attr($x['nam']).'" onerror="this.parentNode.innerHTML=this.alt;" style="display:block;margin:0 auto;max-width:100%;max-height:75px;"></div>'):'')
				.'<span class="member-name">'.esc_html(html_entity_decode($x['nam'],ENT_HTML5|ENT_QUOTES)).'</span>'
				.'<small class="member-tagline" style="display:block;">'.esc_html(html_entity_decode($x['cnm'],ENT_HTML5|ENT_QUOTES)).'</small></a></li>';
	}
